...
function Content() {
    ...
    const getData = async (query) => {
        let dataResult = await axios.get(`https://api.themoviedb.org/3/search/multi?api_key=${process.env.REACT_APP_API_KEY}&query=${query}`);
    ...
    const getMovieTrailer = async (movie) => {
        let trailerResult = await axios.get(`https://api.themoviedb.org/3/movie/${movie.id}/videos?api_key=${process.env.REACT_APP_API_KEY}`);
        ...
        if(trailer === undefined){
            setTrailer(`https://youtube.com/results?search_query=${movie.name.replace
            (/[\u0300-\u036f]/g,'+')}+trailer`)
        }
    }

    const getTvTrailer = async (tv) => {
        let trailerResult = await axios.get(`https://api.themoviedb.org/3/tv/${tv.id}/videos?api_key=${process.env.REACT_APP_API_KEY}`);
        let trailerData = trailerResult.data.results
        for(let i = 0; i < trailerData.length; i++){
            if(trailerData[i].name.toLowerCase().includes('trailer')){
                setTrailer(`https://youtube.com/watch?v=${trailerData[i].key}`)
        ...
        if(trailer === undefined){
            setTrailer(`https://youtube.com/results?search_query=${tv.name.replace
            (/[\u0300-\u036f]/g,'+')}+trailer`)
        }
    }
        ...
        <>
            <Header></Header>
            {
                ...
                <>
                    ...
                    {
                        <>
                            <div className="content__bg" style={{backgroundImage:`url(https://image.tmdb.org/t/p/original${data.backdrop_path})`}}></div>
                            <div className="content__header">
                                    <h1>{data.title || data.original_name}</h1>
                                    <span>
                                        {data.media_type}
                                        {dataType.runtime && ` • ${dataType.runtime} minutes`}
                                        {dataType.number_of_seasons && ` • ${dataType.number_of_seasons == 1 ? `${dataType.number_of_episodes} Episodes` : `${dataType.number_of_seasons} Seasons`}`}
                                        {dataType.first_air_date && ` • ${dataType.first_air_date.split('-')[0]}`}
                                        {dataType.release_date && ` • ${dataType.release_date.split('-')[0]}`}
                                    </span>
                                    <p>{data.overview}</p>
